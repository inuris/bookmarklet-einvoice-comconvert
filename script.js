class ComInfo {
            ...
            if (this.comConfig[key]) {
                ...
                if (configOld) {
                    if (configOld[0] === '{' && configOld[configOld.length - 1] === '}') {
                        template[key].value = this.JSONMerge(configOld, template[key].value);
                    }
    ...
    JSONMerge(str1, str2, overwrite = false) {
        str1 = str1.replace(/'/g, '"');
        str2 = str2.replace(/'/g, '"');
...
async function process() {
    ...
    for (let i = 0; i < ComArr.length; i++) {
        ...
        if (comInfo.comId && comInfo.comName && comInfo.comTaxCode) {
            ...
            document.getElementById("_ext_txtOutput").value += "FAILED" + String.fromCharCode(10);
        }
